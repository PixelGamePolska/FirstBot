...
var getElements = function(query,response){
	...
	var resp={
			...
			try {
				...
				var nbmax=params.nbmax?params.nbmax:'100';
			
				if (params.gadget) {
					p +='&fetch='+$E('{"img":"", "input":""}')+'&proc='+$E('{"img":"","input":""}')+'&gadget=true&price=true&search='+$E(search)+'&regexp='+$E(params.regexp)+'&nbmax='+nbmax;
				} else {
					p +='&fetch='+$E('{"img":"", "input":""}')+'&proc='+$E('{"img":"","input":""}')+'&price=true&search='+$E(search)+'&regexp='+$E(params.regexp)+'&nbmax='+nbmax;		
				};
				...
				var resp2={
						...
						if (response.writeHead) {
							...
							console.log(params.name+'='+JSON.stringify(res)+';');
